...
接收参数：`type`（architecture/feature/changelog/decision/concept）、`topic`（可选）、`working_dir`
...
2. 读取项目说明文件（`CLAUDE.md`、`README.md`、`CLAUDE.md` 等，取第一个存在的）
...
1. 在整个项目中搜索 topic 相关关键词（Grep）
2. 读取命中文件中与 topic 最相关的部分（完整函数/类，不截断）

输出：概念在代码中的体现、关键代码片段（含文件路径和行号）
