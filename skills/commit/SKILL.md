...
2. **Pre-commit checks** (unless `--no-verify`):
   ...
   | Detection | lint | build |
   |-----------|------|-------|
   | `package.json` exists | `npm/yarn/pnpm run lint` (based on lock file) | `npm/yarn/pnpm run build` |
   | `Makefile` exists with `lint`/`build` target | `make lint` | `make build` |
   | `pyproject.toml` / `setup.py` exists | `ruff check .` or `flake8` | skip (Python needs no build) |
   | `go.mod` exists | `go vet ./...` | `go build ./...` |
   | `Cargo.toml` exists | `cargo clippy` | `cargo build` |
...
6. **explain/ directory**: If `explain/` files are changed alongside code, **always split**:
   - First commit: `explain/` files only → `docs(explain): ...`
   - Second commit: remaining code changes → original type
